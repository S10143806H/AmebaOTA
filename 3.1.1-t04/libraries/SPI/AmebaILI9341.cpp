...
void AmebaILI9341::fillRectangle(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color)
{
    ...
    color_hi = color >> 8;
    color_lo = color & 0xFF;
    printf("[DEBUG] transfer color_hi: 0x %x \r\n", color_hi);
    printf("[DEBUG] transfer color_lo: 0x %x \r\n", color_lo);
    color_data = color_hi << 8 | color_lo;
    printf("[DEBUG] transfer color_data: 0x %x \r\n", color_data);
    *portOutputRegister(_dcPort) |= (_dcMask);
