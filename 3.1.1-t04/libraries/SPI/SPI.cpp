...
byte SPIClass::transfer(uint8_t _data, SPITransferMode _mode)
{
    ...
    spi_Index = ((spi_t *)pSpiMaster)->spi_idx;
    // spi_addr = 0x40042000 + (spi_Index * SSI_REG_OFF);
    if (spi_Index == MBED_SPI0) // high speed 50MHz
    {
        spi_addr = SPI0_REG_BASE;
    }
    else if (spi_Index == MBED_SPI1) // normal speed 25MHz
    {
        spi_addr = SPI1_REG_BASE;
    }
    else
    ...
    while (!(HAL_READ32(spi_addr, 0x28) & 0x0000002)) 
        ;
    // HAL_WRITE32(spi_addr, 0x08, 0x1); // disable reg
    // for (int i = 0; i < 63; i++)
    // {
        HAL_WRITE32(spi_addr, 0x60, _data & 0xFFFF); // therefore you can shift data to the reg
        // _data += 1;
    // }
    printf("spi_addr: %x\r\n", spi_addr);
    printf("HAL_WRITE32: %x\r\n", HAL_WRITE32(spi_addr, 0x60, _data & 0xFFFF));

    while (!(HAL_READ32(spi_addr, 0x28) & 0x0000008)) 
        ;
    d = HAL_READ32(spi_addr, 0x60);
    printf("spi_addr: %x\r\n", spi_addr);
    printf("HAL_READ32: %x\r\n", HAL_READ32(spi_addr, 0x60));
    printf("d: %x \r\n", d);
...
// return 0 represents successfully tx
uint16_t SPIClass::transfer16(byte _pin, uint16_t _data, SPITransferMode _mode)
{
    ...
    t.val = _data;
    printf("[DEBUG] transfer16 data1: %x ----------\r\n", _data);
    printf("[DEBUG] transfer16 t.val: %x \r\n", t.val);
    printf("[DEBUG] transfer16 t.lsb: %x \r\n", t.lsb);
    printf("[DEBUG] transfer16 t.msb: %x \r\n", t.msb);
    ...
    printf("[DEBUG] transfer16 data2: %x ----------\r\n", _data);
    printf("[DEBUG] transfer16 t.val: %x \r\n", t.val);
    printf("[DEBUG] transfer16 t.lsb: %x \r\n", t.lsb);
    printf("[DEBUG] transfer16 t.msb: %x \r\n", t.msb);

    // _data = t.msb << 8 | t.lsb; // ADDED BY USER

    _data = t.val;
    printf("[DEBUG] transfer16 data3: %x ----------\r\n", _data);
    printf("[DEBUG] transfer16 t.val: %x \r\n", t.val);
    printf("[DEBUG] transfer16 t.lsb: %x \r\n", t.lsb);
    printf("[DEBUG] transfer16 t.msb: %x \r\n", t.msb);
    return _data;
